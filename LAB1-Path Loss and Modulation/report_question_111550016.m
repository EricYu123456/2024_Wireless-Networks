function [bpsk_throughput_empirical, qpsk_throughput_empirical, qam16_throughput_empirical, qam64_throughput_empirical,...
    bpsk_received, qpsk_received, qam16_received, qam64_received, h, theoretical_SNR_dB...
    ] = lab1(param1, param2)
    % Task 1: Calculate Rx power-----------------------------------------------
    d = param1; % Get link distance from user
    ...
    % Display results
    fprintf('----------------------------------------------------------------------------------------------\n');
    fprintf('Link distance (d) = %g meters, Packet size = %g(bytes)\n', d, param2);
    ...
    % Empirical and theoretical average noise power (in watt and dBm)
    
    theoretical_SNR = Prx_watts / N0_watts;
    theoretical_SNR_dB = 10*log10(theoretical_SNR);
    ...
    Theoretical_noise_power_watts = mean(abs(noise_vec).^2);
    empirical_noise_power_watts = (bpsk_noise_power_watts + qpsk_noise_power_watts + qam16_noise_power_watts + qam64_noise_power_watts) / 4;
    empirical_noise_power_dBm = (bpsk_noise_power_dBm + qpsk_noise_power_dBm + qam16_noise_power_dBm + qam64_noise_power_dBm) / 4;
    empirical_snr = (bpsk_SNR + qpsk_SNR + qam16_SNR + qam64_SNR) / 4;
    empirical_snr_dB = (bpsk_SNR_dB + qpsk_SNR_dB + qam16_SNR_dB + qam64_SNR_dB) / 4;
    % fprintf('Empirical average noise power = %.2e = %.2e (dBm)\n', empirical_noise_power_watts, empirical_noise_power_dBm);
    % fprintf('Theoretical average noise power = %.2e = %.2e (dBm)\n', Theoretical_noise_power_watts, 10*log10(Theoretical_noise_power_watts));
    % fprintf('Empirical SNR = %.2e = %.2e (dB)\n', empirical_snr, empirical_snr_dB);
    % fprintf('Theoretical SNR = %.2e = %.2e (dB)\n', theoretical_SNR, theoretical_SNR_dB);

    % Task 7: Calculate Throughput---------------------------------------------
    sample_duration = 3.2e-6; % 3.2 microseconds
    packet_size = param2; % 500 bytes
    ...
    row_products = prod(qam16_check, 1);
    qam16_num_packets = sum(row_products);
    ...
    optimal_throughput = max([bpsk_throughput_empirical, qpsk_throughput_empirical, qam16_throughput_empirical, qam64_throughput_empirical]);
    fprintf('empirical modulation scheme for link distance d: ');
    if bpsk_throughput_empirical == optimal_throughput
        fprintf('BPSK\n');
    elseif qpsk_throughput_empirical == optimal_throughput
        fprintf('QPSK\n');
    elseif qam16_throughput_empirical == optimal_throughput
        fprintf('16-QAM\n');
    else
        fprintf('64-QAM\n');
    end
...
distances = 50:50:600;
load('SNR_BER.mat');
sample_duration = 3.2e-6;

for i = 1:length(distances)
    d = distances(i);
    packet_len_bit = 4000;% L = 100, 2000, 4000
    packet_len = packet_len_bit / 8;
    [~,~,~,~,~,~,~,~,~,theoretical_SNR_dB] = lab1(d, packet_len);
    column = SNR_BER(:,round(theoretical_SNR_dB));
    bpsk_theoretical_throughput = (1 - column(1))^packet_len_bit / sample_duration;
    qpsk_theoretical_throughput = (1 - column(2))^packet_len_bit / sample_duration*2;
    qam16_theoretical_throughput = (1 - column(3))^packet_len_bit / sample_duration*4;
    qam64_theoretical_throughput = (1 - column(4))^packet_len_bit / sample_duration*6;
    optimal_throughput_theoretical = max([bpsk_theoretical_throughput, qpsk_theoretical_throughput, qam16_theoretical_throughput, qam64_theoretical_throughput]);
    fprintf('theoretical modulation scheme for link distance d: ');
    if bpsk_theoretical_throughput == optimal_throughput_theoretical
        fprintf('BPSK\n');
    elseif qpsk_theoretical_throughput == optimal_throughput_theoretical
        fprintf('QPSK\n');
    elseif qam16_theoretical_throughput == optimal_throughput_theoretical
        fprintf('16-QAM\n');
    else
        fprintf('64-QAM\n');
    end
