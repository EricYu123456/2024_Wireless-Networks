...
function [bpsk_throughput_empirical, qpsk_throughput_empirical, qam16_throughput_empirical, qam64_throughput_empirical,...
    ...
    Empirical_noise_power_watts = mean(abs(noise_vec).^2);
    ...
    fprintf('Empirical average noise power = %.2e (watts) = %.2e (dBm)\n', Empirical_noise_power_watts, 10*log10(Empirical_noise_power_watts)+30);
    fprintf('Theoretical average noise power = %.2e (watts) = %.2e (dBm)\n', 10^((-90 - 30)/10), -90);
    ...
    fprintf('optimal modulation scheme for link distance d: ');
...
for i = 1:length(distances)
    ...
    [bpsk_throughput_empirical, qpsk_throughput_empirical, qam16_throughput_empirical, qam64_throughput_empirical,~,~,~,~,~,~] = lab1(d, 500);
...
% QPSK with 400m
d = 400;
[~,~,~,~,~,qpsk_received,~,~,h,~] = lab1(d,500);
% 提取实部和虚部
real_part = real(qpsk_received ./ h);
imag_part = imag(qpsk_received ./ h);
% 创建新的图形窗口
figure;
% 绘制在复数平面上
plot(real_part, imag_part, '.');
...
% 16QAM with 200m
d = 200;
[~,~,~,~,~,~,qam16_received,~,h,~] = lab1(d,500);
% 提取实部和虚部
real_part = real(qam16_received ./ h);
imag_part = imag(qam16_received ./ h);
% 创建新的图形窗口
figure;
% 绘制在复数平面上
plot(real_part, imag_part, '.');
...
% 64QAM with 100m
d = 100;
[~,~,~,~,~,~,~,qam64_received,h,~] = lab1(d,500);
% 提取实部和虚部
real_part = real(qam64_received ./ h);
imag_part = imag(qam64_received ./ h);
% 创建新的图形窗口
figure;
% 绘制在复数平面上
plot(real_part, imag_part, '.');
